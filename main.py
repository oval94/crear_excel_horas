...
def load_config():
    cfg = {
        "ruta_trabajo": "",            # carpeta donde están los excels
        "nombre_base": DEFAULT_BASE_NAME,
        "rango_borrar": DEFAULT_RANGE,
        "nombre_hoja": ""              # opcional: si vacío usa la primera hoja
    }
...
def find_latest_file(folder: str, base_name: str) -> tuple[str, str] | tuple[None, None]:
    """
    Devuelve (ruta, yyyymm) del último fichero que cumpla:
    ^YYYYMM_<base_name>.xlsx
    """
    ...
    try:
        for fname in os.listdir(folder):
            ...
            if m:
                yyyymm = m.group(1)
                # Validar fecha simple (YYYYMM plausible)
...
def clear_range_in_excel(xlsx_path: str, range_a1: str, sheet_name: str | None):
    ...
    for row in ws.iter_rows(min_row=min_row, max_row=max_row, min_col=min_col, max_col=max_col):
        for cell in row:
            cell.value = None  # elimina el contenido manteniendo el formato
...
def create_next_file(cfg):
    ...
    # 2) Calcular el siguiente YYYYMM
    next_yyyymm = yyyymm_to_next(latest_yyyymm)

    # 3) Construir nombre destino
...
class App(tk.Tk):
    def __init__(self, cfg):
        ...
        tk.Button(self, text="Crear hoja de horas", command=self.on_create, bg="#2e7d32", fg="white").grid(row=4, column=1, sticky="w", **pad)
    ...
    def on_create(self):
        try:
            # Sincronizar en memoria por si el usuario no pulsó "guardar"
            self.cfg["ruta_trabajo"] = self.var_ruta.get().strip()
            self.cfg["nombre_base"] = self.var_base.get().strip() or DEFAULT_BASE_NAME
            self.cfg["rango_borrar"] = self.var_rango.get().strip() or DEFAULT_RANGE
            self.cfg["nombre_hoja"] = self.var_hoja.get().strip()
